...
async function handleDeleteCateg(user: string, categ: string) {
  loading.value = true

  try {
    const res = await apiClient.delete(`/auth/user/${user}/category/${categ}`)
    ...
    if (error.response && error.response.status === 401) {
      ...
      console.error('Erro ao excluir categoria:', error.message || error)
    }
  }
}
</script>
    ...
    <form class="flex flex-col gap-4 space-y-4 overflow-scroll px-8 py-4">
      ...
      <div class="flex w-full flex-row items-center justify-end gap-4">
        ...
        <SubmitButton
          ...
          :on-click="() => handleDeleteCateg(userId, categId)"
          :loading="loading"
          :disabled="loading"
